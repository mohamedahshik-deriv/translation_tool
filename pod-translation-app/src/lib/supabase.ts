...
export async function callEdgeFunction<T>(
    ...
    timeoutMs = 300_000, // 5 minutes — Gemini video processing can be slow
): Promise<T> {
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
...
// Analyze scenes using Gemini via Edge Function
export async function analyzeScenes(videoUrl: string, videoDuration: number) {
    return callEdgeFunction<{
        timecodes: number[];
        scenes: { startTime: number; endTime: number; description: string }[];
    }>('analyze-scenes', { videoUrl, videoDuration });
}

// Match script to scenes using Gemini via Edge Function
...
// Translate text using DeepL via Edge Function
...
// Clone voice using ElevenLabs via Edge Function
