...
import { VideoFile, VideoSegment, SUPPORTED_RESOLUTIONS, VideoResolution } from "@/types";
...
function detectFPS(objectUrl: string): Promise<number> {
    return new Promise((resolve) => {
        ...
        v.oncanplay = () => {
            ...
            const countFrame = (_now: number, meta: { mediaTime: number }) => {
                if (start === null) start = meta.mediaTime;
                frames++;
                if (meta.mediaTime - start < 0.5) {
                    (v as any).requestVideoFrameCallback(countFrame);
                } else {
                    clearTimeout(fallbackTimer);
                    v.pause();
                    const fps = Math.round(frames / (meta.mediaTime - start));
...
function getVideoMetadata(file: File): Promise<{ duration: number; width: number; height: number; frameRate: number }> {
    return new Promise((resolve, reject) => {
        ...
        video.onloadedmetadata = async () => {
            const frameRate = await detectFPS(objectUrl);
...
export function VideoUploader() {
    const { setVideo, setSegments, setCurrentStep } = useAppStore();
                        ...
                        >
                            ...
                            <div className="mt-4 flex justify-center">
                                <button
                                    ...
                                    className="text-xs text-muted-foreground border border-dashed border-border rounded-lg px-4 py-2 hover:border-primary/50 hover:text-primary transition-colors"
                                >
                                    Use Dummy Data (Dev)
                                </button>
                            </div>
