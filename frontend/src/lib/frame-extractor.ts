...
/**
 * Fetch a remote file while reporting download progress (0–1).
 * Falls back to a plain fetch (no progress) if content-length is missing.
 */
...
async function getFFmpeg(
    ...
    if (!loadPromise) {
        ...
        loadPromise = (async () => {
            ...
            // JS file is ~1MB (weight: 10%), WASM file is ~30MB (weight: 90%)
            const coreURL = await fetchBlobURL(
                `${baseURL}/ffmpeg-core.js`,
                'text/javascript',
                onProgress ? (r) => onProgress(r * 0.1) : undefined,
            );
            const wasmURL = await fetchBlobURL(
                `${baseURL}/ffmpeg-core.wasm`,
                'application/wasm',
                onProgress ? (r) => onProgress(0.1 + r * 0.9) : undefined,
            );
